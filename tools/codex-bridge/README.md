...
**整板点阵单显**：内部仍用 6 槽按会话（`session_id` / Cursor `conversation_id`）记录状态，
渲染时取**优先级最高的活跃会话**，把整块可见轴灯当成一块 **4 列 × 6 行点阵**
（`AXIS_LAYOUT`，主字区 = 左侧 3 列 × 6 行 = 18 灯，右侧列 c3 的 `6,7,8,9` 做侧条），
用该会话的**平台色 + 状态图形/动画**显示；空闲/长时间无活动时整板熄灭。即「色相按平台，
图形按状态，序号按任务，一次只显示一个平台」。详见
...
- **任务序号**：每平台一个自增计数器，新会话绑定槽位时分配 `1..9`（循环）；完成时以点阵数字显示，
  「形状=第几个任务，颜色=哪个平台」。
- **c3 侧条**：主区显示某平台时，其它仍活跃（working/需输入/error/未读）的平台在右侧列
  `6,7,8,9` 各点一格平台色（半亮），提示「后台还有别的平台在」。
- **字模/图标**：`glyphs.ts` 含 `0–9 / ? / ! / ✓ / 笑 / 哭 / 心跳`（3×5）；emoji 仅做状态图标语义。

> `Notification` 在 CodeBuddy/Claude 里也用于 60s 空闲提醒。为避免**已完成/空闲**会话被
> 空闲提醒误翻成琥珀（整板卡黄闪），`requiresInput` **只在会话正处于 working 时才被接受**；
> 落在非 working 会话上的 `Notification` 视为空闲提醒忽略。

整板显示的优先级仲裁（进行中排在完成之前，注意态最高）：
`error > requiresInput > working > completeUnread`；同级按最近更新时间取最新平台。
底灯 `0..5`（正面不可见）跟随整板同色，亮度乘 underglow 系数。

### 自动熄灭与唤醒（Codex Micro 式）

当**没有任何 working/需输入/error** 任务、且距上一次事件超过 `idleOffMs`（默认 3 分钟）时，
整板（含 soft-unread 微光）**全部熄灭**省电；任何新事件立即唤醒。注意这与「完成后 5s 降亮」
是两回事：5s 是单会话完成态的降亮，3 分钟是整板全局待机熄屏。

### 已知行为：未标记平台会话

升级到平台色方案前安装的 hooks（forwarder 没带平台参数）发来的事件会被标记为
`unknown`（灰白）。这类会话同样参与整板仲裁：若它卡在 working，会在其它平台空闲时把整板
显示成灰白。**修复办法**：对该工具重新 `install-hooks <target>`（会写入带平台参数的
forwarder 命令）。

## hooks 路线的固有限制（务必知晓）

- **error（黄闪）状态受限**：Codex / Claude 家族 hooks 没有独立失败事件，`Stop` 无论成功
  失败都触发，无法显示 error。Cursor 的 `stop` 带 `status`，`error` 时可黄闪；审批琥珀闪
...
src/
  ...
  types.ts          # 状态枚举、平台色、优先级、任务序号
  effects.ts        # 呼吸/快闪亮度系数
  glyphs.ts         # 3×5 字模：0-9 / ? / ! / ✓ / 笑哭心跳 + 点阵映射
  snake.ts          # working 贪吃蛇动画器
  threadStore.ts    # 6 槽状态机、绑定、仲裁、点阵渲染、侧条、自动熄灭
