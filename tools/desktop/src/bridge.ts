...
/**
 * 监管现有 agent-bridge 守护进程：以子进程方式 spawn `node dist/index.js`，
 * 汇聚日志、解析状态、崩溃自动重启（指数退避）；并提供一次性的 build /
 * install-hooks / uninstall-hooks 调用。第一阶段完全不改动 agent-bridge 本身。
 */
export class BridgeController extends EventEmitter {
  ...
  bridgeDir(): string {
    ...
    // dist/bridge.js -> tools/desktop/dist -> tools/desktop -> tools -> tools/agent-bridge
    return resolve(__dirname, "..", "..", "agent-bridge");
  }
  ...
  async start(): Promise<void> {
    ...
    if (!existsSync(this.entry())) {
      logStore.append(
        `[desktop] 未找到 ${this.entry()}，尝试自动构建 agent-bridge…`,
      );
      const built = await this.build();
      if (!built || !existsSync(this.entry())) {
        logStore.append(
          "[desktop] 构建失败或产物缺失。请先在 tools/agent-bridge 执行 `npm install && npm run build`。",
        );
        this.setState("error");
        return;
      }
  ...
  /**
   * 一次性构建 agent-bridge（及其工作区依赖 @planckeys/led-protocol，拓扑有序）。
   * 走 pnpm workspace 过滤器；需要已 `pnpm install`。
   */
  build(): Promise<boolean> {
    return new Promise((res) => {
      logStore.append("[desktop] 运行 pnpm 构建 agent-bridge（含 led-protocol）…");
      const pnpm = process.platform === "win32" ? "pnpm.cmd" : "pnpm";
      let child: ChildProcess;
      try {
        child = spawn(
          pnpm,
          ["--filter", "@planckeys/agent-bridge...", "run", "build"],
          {
            cwd: this.bridgeDir(),
            env: this.childEnv(),
          },
  ...
  runHooks(action: HookAction, target: HookTarget): Promise<boolean> {
    return new Promise((res) => {
      if (!existsSync(this.entry())) {
        logStore.append(
          `[desktop] ${action} 前需要先构建 agent-bridge（${this.entry()} 不存在）。`,
        );
        res(false);
        return;
      }
