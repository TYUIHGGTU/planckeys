...
- **后台状态灯（codex-bridge）**：菜单栏常驻、开机自启，点托盘即可启停 / 切灯效 /
  装 hooks / 看日志——**告别每次手动 `npm start`**。
- **控制台窗口（dashboard）**：托盘「打开控制台（灯效 / 改键）」弹出完整的
  **灯效编辑 + 实时改键** 界面（复用 `tools/dashboard`，WebHID 控灯 + Web Serial 改键）。

> 集成方式：Electron 外壳**子进程监管** codex-bridge，**窗口内嵌** dashboard 的已构建产物；
> 目前**不改动** `codex-bridge` / `dashboard` 两个工程本身。仍待做：把控灯 HID 收拢到主进程做
> 统一仲裁（见「路线图」）。
...
桌面 App (Electron 主进程)
  ...
  ├─ spawn `node dist/index.js`      ->  codex-bridge 守护进程
...
- 后台状态灯：App 只负责**编排**（启停/重启/崩溃退避/汇聚日志/透传 `CODEX_BRIDGE_*` 配置），
  收事件与驱动灯仍由 codex-bridge 完成，逻辑零改动。
- 控制台：窗口加载 dashboard 的静态产物；Electron 默认要求原生设备选择器回调，App 已自动
  挑中 `PlanckKeys` HID 与 ZMK 串口，体验接近网页版。

## 前置条件

- macOS + Node.js ≥ 18（沿用 codex-bridge 的要求）。
- **先构建 codex-bridge**（App 会 spawn 它的 `dist/index.js`）：

```bash
cd ../codex-bridge && npm install && npm run build
```

- **先构建 dashboard**（控制台窗口加载它的 `dist/index.html`）：

```bash
cd ../dashboard && npm install && npm run build
```

  两者若忘了构建，App 会在需要时尝试自动 `npm run build`（需其 devDependencies 已安装）。
  控制台也可指向 dashboard 的开发服务器：先 `cd ../dashboard && npm run dev`，再以
  `PLANCKEYS_DASHBOARD_URL=http://localhost:5173 npm run dev` 启动本 App。
...
- **working 灯效**：snake / spinner / equalizer / breathe（切换后自动重启子进程生效）。
- **亮度**：64–255。
- **安装 / 卸载 hooks**：codex / codebuddy / workbuddy / claude / cursor（一次性调用，结果弹通知）。
  - Codex 仍需在 `codex` 里 `/hooks` 手动 trust；Cursor/CodeBuddy 等见 codex-bridge README。
...
- **HID 独占**：同一时刻只能有一个进程打开左板。若你**另外**还手动跑着 codex-bridge，
...
1. **套壳 + 常驻**（已完成）：菜单栏监管 codex-bridge，开机自启，消灭命令行启动。
...
src/
  ...
  bridge.ts        # BridgeController：spawn/监管 codex-bridge 子进程、重启、hooks
